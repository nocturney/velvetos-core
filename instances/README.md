...
Both `nocturney/velvetos-core` and `nocturney/velvetos-velvet-factory` are **public** — Cloud Agents can **read** and `attach-core`. This agent cannot `createRepository`. Setup gaps: `docs/ACCESS-GAPS.md`. Push may need owner PAT or `cursor[bot]` write on the target repo.
