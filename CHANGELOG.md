...
- 2026-08-31 — **3D AI Studio MCP**: `3daistudio` HTTP server at `https://mcp.3daistudio.com/mcp` in `.cursor/mcp.json`. Desk `threedaistudio` on production seat. Playbooks `vfprod/CONNECT-3DAI.md` + `3DAISTUDIO.md`. OAuth Desktop only; no API key in git. מפת [awesome-agent-orchestrators](https://github.com/andyrewlee/awesome-agent-orchestrators) (194) על `vfe2b` + `vfharness` — דופק / אימות / ארטיפקט / תקרת לולאה. בלי ADE שני, בלי amux, בלי OpenClaw. ייעול משמרת: בריף 07:00 נשלח בג׳ימייל; פנייה נקובה ב־`reply`; תוכן דרך `SEND.md`; `decision_gate` = ₪ בלבד. Orchestrator overlay on existing crews; no second runtime. See `docs/ORCHESTRATORS.md`.
