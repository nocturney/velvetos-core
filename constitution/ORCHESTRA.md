...
| **Cloud Agent** — גוגל «already signed in on another device» | WebSearch + פקים על הדיסק **עכשיו** | לראש צוות: אחרי 48 שעות קישור ב־`nocturney@gmail.com` (גם ספאם) «help you sign in here». אין סיסמה בצ׳אט. אין אורח Flash-Lite. אין גוף מומצא. |
| שני שולחנות נפלו | השולחן הפתוח נושא את כל העומס עכשיו | מטמיעים רק מגוף אמיתי |
| שלושתם נפלו | פקים שכבר על הדיסק + בלוק `05` = «אין חדש במשרד» **רק אם גם אין הטמעה מ-WebSearch/פקים** | הסלמה לראש צוות על חומות · בלי גוף מומצא |
