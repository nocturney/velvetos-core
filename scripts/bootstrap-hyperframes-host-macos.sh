...
HYPERFRAMES_VERSION="$(python3 scripts/vf_toolchain.py get components.hyperframes.version)"
NODE_VERSION_PIN="$(python3 scripts/vf_toolchain.py get components.node.version)"
FFMPEG_STATIC_PACKAGE="$(python3 scripts/vf_toolchain.py get components.ffmpegStatic.package)@$(python3 scripts/vf_toolchain.py get components.ffmpegStatic.version)"
...
install_user_node() {
  ...
  [[ -x "$node_dir/bin/corepack" ]] && ln -sf "$node_dir/bin/corepack" "$HOME/.local/bin/corepack"
...
if [[ -z "$NODE_VERSION" || -z "$NODE_MAJOR" || "$NODE_MAJOR" -lt 22 ]]; then install_user_node; fi
...
if ! command -v ffmpeg >/dev/null 2>&1 || ! command -v ffprobe >/dev/null 2>&1; then install_user_ffmpeg; fi
...
if [[ "$CURRENT_HF" != "$HYPERFRAMES_VERSION" && "$CURRENT_HF" != "v$HYPERFRAMES_VERSION" ]]; then fail "HyperFrames version mismatch after install: expected $HYPERFRAMES_VERSION, got $CURRENT_HF"; fi
...
{"jobId":"host-smoke","backend":"hyperframes","projectDir":"$SMOKE_ROOT","composition":"index.html","stage":"review","target":"review","format":"mp4","resolution":"portrait","fps":30,"quality":"standard","output":"renders/host-smoke.mp4","audioRequired":false,"strictAll":false}
JSON
...
state = {"schemaVersion":1,"hostId":host_id,"hostname":socket.gethostname(),"role":"velvetos-render-host","backend":"hyperframes","hyperframesVersion":version,"repoCommit":commit,"smokeReceiptSha256":receipt.get("sha256"),"smokeVerifiedAt":receipt.get("verifiedAt"),"registeredAt":datetime.now(timezone.utc).isoformat(),"status":"host-smoke-verified"}
Path(state_path).write_text(json.dumps(state, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
PY
say "OK HyperFrames host smoke verified host=$HOST_ID hyperframes=$HYPERFRAMES_VERSION"

if command -v agent >/dev/null 2>&1; then agent status || fail "Cursor agent is installed but not logged in; run 'agent login' and rerun bootstrap"; else fail "Cursor agent CLI missing"; fi
if pgrep -f "agent worker.*${HOST_ID}" >/dev/null 2>&1; then say "OK Cursor worker '$HOST_ID' is already running"; exit 0; fi
if [[ "$START_WORKER" -eq 1 ]]; then exec env PATH="$HOME/.local/bin:$PATH" HYPERFRAMES_NO_UPDATE_CHECK=1 HYPERFRAMES_NO_AUTO_INSTALL=1 NPM_CONFIG_PREFIX="$HOME/.local" agent worker --name "$HOST_ID" start; fi
say "HOST READY, WORKER NOT RUNNING"
say "Start with: bash scripts/bootstrap-hyperframes-host-macos.sh --start-worker"
