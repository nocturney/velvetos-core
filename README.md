...
<div dir="rtl"><strong>שינוי הטמעה אחרון:</strong> 2026-09-18: **Instagram delivery deploy hardening:** production deploy paths now fail closed unless project/region remain `instamcp` / `me-west1`, the optional issuer bearer is empty or canonical, Cloud Build o…</div>
<div dir="ltr"><strong>Latest implementation change:</strong> 2026-09-18: **Instagram delivery deploy hardening:** production deploy paths now fail closed unless project/region remain `instamcp` / `me-west1`, the optional issuer bearer is empty or canonical, Cloud Build o…</div>
...
<tr><td><strong>OpenPost Publishing Control Plane</strong></td><td><strong>PROD OWNER READY / PROVIDER BLOCKED</strong></td><td dir="rtl" align="right">OpenPost v4.35.0 רץ על host ייעודי ב־GCP עם HTTPS מאומת. owner/workspace מאומתים, המשתמש הראשון instance-admin והרשמות חדשות כבויות. ראיות ה־bootstrap בריפו כוללות כעת פקודות אימות, שאילתות read-only ו־hashes שניתנים לשחזור; אין בכך שינוי runtime או LIVE. Meta App Secret/provider OAuth ו־write חי מאומת עדיין חסרים.</td></tr>
...
<tr><td><strong>OpenPost Publishing Control Plane</strong></td><td><strong>PROD OWNER READY / PROVIDER BLOCKED</strong></td><td>OpenPost v4.35.0 runs on the dedicated GCP origin with verified HTTPS. Owner/workspace bootstrap is verified, the first user is instance admin, and new registrations are disabled. The repository now carries replayable bootstrap evidence with verification commands, read-only queries, and bound hashes; this does not change runtime or LIVE state. Meta App Secret/provider OAuth and a canonically verified live write remain pending.</td></tr>
