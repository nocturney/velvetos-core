...
<tr><td><strong>Instagram Delivery Approval</strong></td><td><strong>IMPLEMENTED / NEEDS_OPERATOR_SETUP</strong></td><td dir="rtl" align="right">כתיבות Instagram דורשות receipt חתום מ־issuer מבודד; חשבון השירות של ה־issuer הוא `velvet-delivery-issuer`, ו־GCS atomic spend חוסם replay. פריסת GCP חיה ואימות end-to-end עדיין דורשים operator evidence.</td></tr>
...
<tr><td><strong>Instagram Delivery Approval</strong></td><td><strong>IMPLEMENTED / NEEDS_OPERATOR_SETUP</strong></td><td>Instagram writes require a signed receipt from the isolated issuer; the issuer service account is `velvet-delivery-issuer`, and atomic GCS spend blocks replay. Live GCP deployment and end-to-end verification still require operator evidence.</td></tr>
