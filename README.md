...
<div dir="rtl"><strong>שינוי הטמעה אחרון:</strong> 2026-09-18 — **Structured demand signals in vfresearch:** registered `cporter202/API-mega-list` and `cporter202/social-growth-apis-for-creators` as discovery-only catalogues; added provider-agnostic `DemandSign…</div>
<div dir="ltr"><strong>Latest implementation change:</strong> 2026-09-18 — **Structured demand signals in vfresearch:** registered `cporter202/API-mega-list` and `cporter202/social-growth-apis-for-creators` as discovery-only catalogues; added provider-agnostic `DemandSign…</div>
...
<tr><td><strong>OpenPost Publishing Control Plane</strong></td><td><strong>STAGING v4.35.0 + PROD HOST READY / PROVIDER BLOCKED</strong></td><td dir="rtl" align="right">בתוך `vfigos`: Windows staging נשאר נעוץ ב־v4.35.0. נוסף host קבוע ב־GCP עם Linux artifact SHA-256 מאומת, HTTPS/TLS ו־`/api/v1/ready=200` אחרי reset מלא. production promotion עדיין חסום על owner/workspace bootstrap, Meta App Secret/provider OAuth ו־write חי מאומת.</td></tr>
...
<tr><td><strong>OpenPost Publishing Control Plane</strong></td><td><strong>STAGING v4.35.0 + PROD HOST READY / PROVIDER BLOCKED</strong></td><td>Inside `vfigos`: Windows staging remains pinned to v4.35.0. A stable GCP host now runs the verified Linux artifact with HTTPS/TLS and `/api/v1/ready=200` after a full reset. Production promotion remains blocked on owner/workspace bootstrap, Meta App Secret/provider OAuth, and a verified live write.</td></tr>
