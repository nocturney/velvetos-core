...
<div dir="rtl"><strong>שינוי הטמעה אחרון:</strong> 2026-09-18 — **OpenPost Instagram provider OAuth:** saved the Meta provider app in production OpenPost, restarted the service, and verified startup loaded one database provider app and registered the Instagram …</div>
<div dir="ltr"><strong>Latest implementation change:</strong> 2026-09-18 — **OpenPost Instagram provider OAuth:** saved the Meta provider app in production OpenPost, restarted the service, and verified startup loaded one database provider app and registered the Instagram …</div>
...
<tr><td><strong>Structured Demand Signals</strong></td><td><strong>IMPLEMENTED / PROVIDER UNPROVEN</strong></td><td dir="rtl" align="right">בתוך <code>vfresearch</code>: WebSearch/WebFetch נשארים קו ראשון; ראיות ציבוריות מובנות יכולות להתנרמל ל־DemandSignalPacket עם מקור/ספק/שאילתה/geo/חלון/metrics/series בלי להמציא missing values או demand score. אין scheduler/runtime/SoT חדש, אין auto-SKU ואין סמכות מחיר; provider חי נשאר UNPROVEN עד run אמיתי עם credentials.</td></tr>
<tr><td><strong>Media Vault + vfmedia</strong></td><td><strong>IMPLEMENTED</strong></td><td dir="rtl" align="right">incoming → source → WIP → approved, עם checksum ו־persist-before-move; Asset Truth האופציונלי מאומת מול `catalog.schema.json`, ושדות/ערכים לא קנוניים נכשלים סגורה.</td></tr>
...
<tr><td><strong>Gmail Operating Brief</strong></td><td><strong>IMPLEMENTED / EVIDENCE-GATED</strong></td><td dir="rtl" align="right">בריף 09:00 וה־10:00 Delivery Guard מקבעים קודם artifact יומי קנוני דרך `vfops_loop.py brief --write --date`; backfill אינו receipt למסירת Gmail. מקורות weekly/growth מיושנים מסומנים כלא-החלטה-להיום, ו־Research Seat ב־`no_meaningful_findings` מפיק `אין חדש במשרד`; HTML/שליחה נשארים שלבים נפרדים ורק עם credentials ו־Visible Text evidence תקפים.</td></tr>
...
<tr><td><strong>Structured Demand Signals</strong></td><td><strong>IMPLEMENTED / PROVIDER UNPROVEN</strong></td><td>Inside <code>vfresearch</code>: WebSearch/WebFetch remain first-line; structured public evidence can normalize into DemandSignalPacket with source/provider/query/geo/window/metrics/series while preserving missing values and forbidding synthetic demand scores. No new scheduler/runtime/SoT, no automatic SKU promotion, and no pricing authority; live provider execution remains UNPROVEN until a real credentialed run.</td></tr>
<tr><td><strong>Media Vault + vfmedia</strong></td><td><strong>IMPLEMENTED</strong></td><td>Incoming → source → WIP → approved with checksums and persist-before-move; optional Asset Truth is validated against `catalog.schema.json`, while non-canonical fields or values fail closed.</td></tr>
...
<tr><td><strong>Gmail Operating Brief</strong></td><td><strong>IMPLEMENTED / EVIDENCE-GATED</strong></td><td>The 09:00 brief and 10:00 Delivery Guard persist the canonical same-day artifact through `vfops_loop.py brief --write --date`; a backfill is not a Gmail delivery receipt. Stale weekly/growth inputs remain non-current decisions, and `no_meaningful_findings` yields the explicit `אין חדש במשרד` empty state; rendering/sending remain separate stages requiring valid credentials and Visible Text evidence.</td></tr>
