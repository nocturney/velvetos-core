...
<tr><td><strong>HyperFrames Video Backend</strong></td><td><strong>IMPLEMENTED / MAC VERIFIED / WINDOWS PENDING</strong></td><td dir="rtl" align="right">`Mac-Office` / `sderot-mac` נשאר ה־render host המועדף ומאומת ב־smoke. `Windows-Office` / `sderot-windows` מוגדר כ־fallback אוטומטי לפי `first-healthy-verified-host`, עם bootstrap ללא admin ו־Cursor worker native; הוא ייכנס ל־failover רק אחרי smoke+receipt אמיתיים על ה־Windows. אין tunnel/port נכנס ואין הורדת QA.</td></tr>
...
<tr><td><strong>HyperFrames Video Backend</strong></td><td><strong>IMPLEMENTED / MAC VERIFIED / WINDOWS PENDING</strong></td><td>`Mac-Office` / `sderot-mac` remains the preferred smoke-verified render host. `Windows-Office` / `sderot-windows` is configured as the automatic `first-healthy-verified-host` fallback with a no-admin bootstrap and native Cursor worker; it becomes failover-eligible only after a real Windows smoke render + receipt. No inbound tunnel/port and no QA relaxation.</td></tr>
