...
<div dir="rtl"><strong>שינוי הטמעה אחרון:</strong> 2026-09-15 — **Instagram text-routing tighten:** `send`/`push`/`add` … to Instagram fail-closed through `instagram_action` delivery evidence; noun `post` and office `share … analytics` no longer false-trip publ…</div>
<div dir="ltr"><strong>Latest implementation change:</strong> 2026-09-15 — **Instagram text-routing tighten:** `send`/`push`/`add` … to Instagram fail-closed through `instagram_action` delivery evidence; noun `post` and office `share … analytics` no longer false-trip publ…</div>
