...
<tr><td><strong>VF Creative Publication Preflight</strong></td><td><strong>IMPLEMENTED / FAIL-CLOSED</strong></td><td dir="rtl" align="right">ולידטורי ה־preflight וה־publication evidence קשורים כעת ל־Contract 6 / Revision 6.4; רק שלושת רפרנסי האסתטיקה הנוכחיים נספרים כ־style, ו־Product Truth guide נשאר QA בלבד. אימות ה־hash סובל נרמול CRLF בלי להתיר drift בסמכות. בנוסף, Product Truth teaching-sheet הישן חסום לפי SHA ו־product_truth_lock חייב להיות קשור ל־guide המדויק של v6.4.</td></tr>
...
<tr><td><strong>Media Vault + vfmedia</strong></td><td><strong>IMPLEMENTED</strong></td><td dir="rtl" align="right">incoming → source → WIP → approved → published archive, עם checksum ו־persist-before-move. שלב `05 - פורסם` שומר את הנגזרת המדויקת שפורסמה; מעבר `04→05` מותר רק אחרי Instagram `published_verified` / `liveVerified` עם media ID + permalink, והמקור נשאר ב־`02 - מקור`. Backfill היסטורי מאומת קיים ל־G001/G002 בלבד; G003 נשאר מחוץ לארכיון עד התאמת קובץ מדויקת. Asset Truth ו־publication evidence מאומתים מול `catalog.schema.json`, ושדות/ערכים לא קנוניים נכשלים סגורה.</td></tr>
