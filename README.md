...
<tr><td><strong>OpenPost Publishing Control Plane</strong></td><td><strong>STAGING v4.34.2 / PRODUCTION BLOCKED</strong></td><td dir="rtl" align="right">???? `vfigos`: Windows staging ???? ??v4.34.2 ?? SHA-256 ?????; backup + migration smoke 134?136 + integrity ????, diagnostics ????. production ???? ?? public HTTPS/Meta OAuth ??? delivery approval ??.</td></tr>
<tr><td><strong>Instagram Delivery Approval</strong></td><td><strong>DEPLOYED / NEEDS_OPERATOR_SETUP</strong></td><td dir="rtl" align="right">??issuer ???mutation runtime ?????? ?? identities ??????; ????? owner-invoker ??? ???? token, ?? `/healthz` ??revision ??? ????? ????? 404 ???? ????? ????? issue?mutation?replay-block ????. ?? write ???? ???? ??? `velvet.delivery_approval.v1` ???.</td></tr>
