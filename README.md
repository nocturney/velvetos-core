...
<div dir="rtl"><strong>שינוי הטמעה אחרון:</strong> 2026-09-17 ? **Delivery-approval IAM probe semantics:** removed the nonexistent `iam.serviceAccountKeys.upload` probe; Google IAM authorizes the `projects.serviceAccounts.keys.upload` method with `iam.serviceAc…</div>
<div dir="ltr"><strong>Latest implementation change:</strong> 2026-09-17 ? **Delivery-approval IAM probe semantics:** removed the nonexistent `iam.serviceAccountKeys.upload` probe; Google IAM authorizes the `projects.serviceAccounts.keys.upload` method with `iam.serviceAc…</div>
...
<tr><td><strong>OpenPost Publishing Control Plane</strong></td><td><strong>STAGING v4.34.2 / PRODUCTION BLOCKED</strong></td><td dir="rtl" align="right">בתוך `vfigos`: Windows staging נעוץ ב־v4.34.2 עם SHA-256 מאומת; backup + migration smoke 134→136 + integrity עברו, diagnostics כבוי. production חסום על public HTTPS/Meta OAuth ועל delivery approval חי.</td></tr>
<tr><td><strong>Instagram Delivery Approval</strong></td><td><strong>DEPLOYED / NEEDS_OPERATOR_SETUP</strong></td><td dir="rtl" align="right">ה־issuer וה־mutation runtime פרוסים עם identities נפרדים; מסלול owner-invoker הצר מפיק token, אך `/healthz` ב־revision החי עדיין מחזיר 404 ואין עדיין הוכחת issue→mutation→replay-block מלאה. כל write נשאר חסום בלי `velvet.delivery_approval.v1` תקף. Cloud Build נעול ל־`velvet-vfigos-builder` בהרשאות least-privilege, וה־default build identity נשאר כפוף ל־effective-IAM deny checks.</td></tr>
...
<tr><td><strong>OpenPost Publishing Control Plane</strong></td><td><strong>STAGING v4.34.2 / PRODUCTION BLOCKED</strong></td><td>Inside `vfigos`: exact v4.34.2 Windows staging with verified SHA-256; backup + migration smoke 134→136 + integrity passed; diagnostics are off. Production is blocked on public HTTPS/Meta OAuth and live delivery approval.</td></tr>
<tr><td><strong>Instagram Delivery Approval</strong></td><td><strong>DEPLOYED / NEEDS_OPERATOR_SETUP</strong></td><td>Issuer and mutation runtime are deployed with separated identities; the narrow owner-invoker path mints a token, but the live revision still returns 404 for `/healthz` and full issue→mutation→replay-block proof is not complete. Every write remains blocked without a valid `velvet.delivery_approval.v1`. Cloud Build is pinned to the least-privilege `velvet-vfigos-builder`; the project default build identity remains subject to effective-IAM deny checks.</td></tr>
