...
| **Instagram (canonical)** | Publish image/carousel/reel/**story**, Insights, feed read via Graph API. **ready** (Cloud Run Streamable HTTP live). Project `instamcp` · `me-west1` · `velvet-instagram-mcp` · bearer · scale-to-zero. `remote_access: ready`. Local stdio = fallback (`DEPLOY-CODESPACE.md`). [`packages/vfigos/CONNECT-IG.md`](../packages/vfigos/CONNECT-IG.md). Package `adelaidasofia-instagram-mcp`. Not in project `mcp.json` (secrets) | `vfigos`, `vfinsights`, `vfgrowth` |
...
### 0. Instagram Publish + Insights + Stories — adelaidasofia/instagram-mcp · **canonical (ready; Cloud Run live)**
...
Not in project `mcp.json` (secrets). Cloud Run live (`instamcp` / `me-west1` / `velvet-instagram-mcp`). Evidence: `packages/vfigos/live/remote-health.json`. Team MCP: `packages/vfmcp/mcp.cloud.example.json`. Codespace stdio = fallback. Fly.io = legacy (`LEGACY-FLY.toml`).
...
Failover until live remote MCP: Canva export → Drive `create_file` → Gmail `send_message` same turn. Live claim requires `list_media`/`get_media` verify. Watchdog surfaces `ig_remote_pending` / `ig_remote_degraded` — do not publish; keep queue.
