...
| 02 | כסף בעבודה | הזמנות וחוב שכבר במערכת + **ספר:** `python3 scripts/vfbooks.py brief` + **עלות חומר:** `python3 scripts/vfcost.py brief` דרך `vfops_loop.py` | ₪ מומצא, מייל ללקוח, מחיר מכירה בלי ראש צוות |
| 03 | מה להדפיס ולפרסם | באצ׳ אחרי סלייס, רילס בלי מק״ט, **מדף:** `vfsku.py brief` + `vfsku.py scan` + `vfsku/week.md` + `vfprod.py print-done` דרך `vfops_loop.py brief` | מחיר בלי סכום מראש צוות, שם MakerWorld מהאוויר |
