...
| 02 כסף בעבודה | הזמנות, חוב, שולם מ־`vfbooks/data/orders.json` + Invoice4U snapshot (אין ספירה אם ריק; לא inbox) + שורת עלות חומר `python3 scripts/vfcost.py brief` דרך `vfops_loop.py` (בלי ₪ מכירה) |
| 03 מה להדפיס ולפרסם | מוכנים להדפסה + שעות תור מאומתות + הצעות `#vfresearch` + מדף `python3 scripts/vfsku.py brief` + `vfsku.py scan` + `vfsku/week.md` + צי `python3 scripts/vfprod.py brief` דרך `python3 scripts/vfops_loop.py brief` + חנות מחר `python3 scripts/vfsku.py shop` |
