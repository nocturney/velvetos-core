...
| כלי שם | כאן | דין |
|---|---|---|
| שליחת אינסטגרם | Canva + **adelaidasofia/instagram-mcp** (`CONNECT-IG.md` · `REMOTE.md`) או failover `vfigos/SEND.md` | **canonical** — Cloud Run Streamable HTTP **live** (`instamcp` / `me-west1` / `velvet-instagram-mcp`, min=0) · **`remote_access: ready`** אחרי healthcheck; stdio = fallback מקומי; failover Drive + Gmail כש־degraded |
...
11. **Instagram MCP (canonical)** — [`adelaidasofia/instagram-mcp`](https://github.com/adelaidasofia/instagram-mcp) רשום ב־`core-mcp.json`. פלייבוק `vfigos/CONNECT-IG.md` + `REMOTE.md` (Cloud Run ייצור) + `DEPLOY-CODESPACE.md` (fallback). סטטוס שולחן `ready` (auth ready · transport streamable-http · remote_access ready). Cloud Run live. Stories + Insights על אותו MCP. DM כבוי. Metricool לא נדרש. failover: Canva+Drive+Gmail כש־degraded. **לגאסי:** jlbadano/ig-mcp · Fly.toml → `LEGACY-FLY.toml` · Codespace stdio.
...
| פער | למה לא |
|---|---|
| Publish MCP לאינסטגרם **חי תמיד** (Cloud remote) | **קנוני ממופה** → [adelaidasofia/instagram-mcp](https://github.com/adelaidasofia/instagram-mcp) + עטיפת `vfigos/remote/` (Streamable HTTP). Auth+stdio מאומתים ב־Codespace. Deploy path: **Google Cloud Run** `me-west1` min=0 (`REMOTE.md`). **`remote_access: ready`** (healthcheck 2026-09-09). Failover: Canva+Drive+Gmail כש־degraded. verify: `list_media`/`get_media`. Meta DevTools ≠ Publish. jlbadano = legacy. Fly = legacy |
