...
| כלי שם | כאן | דין |
|---|---|---|
| שליחת אינסטגרם | Canva + **adelaidasofia/instagram-mcp** (`CONNECT-IG.md`) או failover `vfigos/SEND.md` | **canonical** — Cloud Team MCP `instagram` מאומת 2026-09-09 (`remote_access: ready`); failover Drive + Gmail אותו תור |
...
| פער | למה לא |
|---|---|
| Publish MCP לאינסטגרם **חי תמיד** (Cloud remote) | **קנוני מאומת** → [adelaidasofia/instagram-mcp](https://github.com/adelaidasofia/instagram-mcp) (`CONNECT-IG.md`). Cloud Team MCP namespace `instagram` · **`remote_access: ready`** (2026-09-09 live_check). Failover: Canva+Drive+Gmail. verify: `list_media`/`get_media`. Meta DevTools ≠ Publish. jlbadano = legacy |
