...
**חוזה יום (fail-closed בסנסור):** כל בלוק שכותרתו `### YYYY-MM-DD` (אופציונלי טקסט אחרי התאריך) חייב לכלול את שלושת השדות החובה. בלי Pydantic — Markdown + `check-vfops-loop.py`.

| שדה | חובה | כלל |
|---|---|---|
| `**מושב:**` | כן | מושב אחד או יותר מהדסק |
| `**למדנו:**` | כן | לפחות שורה אחת — עובדה/העדפה |
| `**מקור:**` | כן | שיחה / מייל / רצפה / נתיב ארטיפקט — לא המצאה |
| `**מחר:**` | לא | פעולה אחת |
| `**נסגר:**` / `**פתוח:**` | לא | סגירת רשומה ישנה |
...
בלוק catch-up (`## רטרו ראשוני`) פטור מהחוזה — רק בלוקי `### YYYY-MM-DD`.
