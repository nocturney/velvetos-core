...
When live GitHub/repository access or the repository executable preflight is unavailable inside a ChatGPT Project, a hash-verified Project bundle at **Contract 6 / Revision 6.4 or newer** may satisfy the baseline authority and creative/public-copy routing for `creative_publication` only. Record `repo_state=UNAVAILABLE_PROJECT_BUNDLE_FALLBACK` and the exact bundle ID/hashes. Do not claim repository synchronization, deployment or a repository preflight PASS.
