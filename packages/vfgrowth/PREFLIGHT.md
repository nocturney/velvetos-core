...
שער עריכה: `EDIT-GATE.md`. קול: `vfcopy/VOICE.md`. שכנים: `vfcopy/VOICE-RESEARCH.md`.  
מסירה: `HANDOFF-he.md`.
...
| # | בדיקה | עובר רק אם |
|---|---|---|
| א | **VOICE.md** | מצב אחד: תהליך-קצר **או** סיפור-מוצר. בלי פתיחת מגבלה / «מוכנים» / «בלי משלוח» / קופי דק |
| ב | **Canva / vfcovers** | `edit_url` אמיתי מ־Canva MCP **או** PNG מ־`vfcovers` / `vfcanva` (`compose_slides.py` / `studio/render.py`). לא JPEG גולמי |
...
חסר אחד מהארבעה = **נכשל-סגור**. אין «כמעט» ואין שיבוץ על תנאי.
