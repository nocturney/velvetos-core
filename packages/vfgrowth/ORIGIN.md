# vfgrowth

Growth pack — content sprints and acquisition work.

| | |
|---|---|
| Agent | https://cursor.com/agents/bc-e68393a0 |
| bcId | `bc-e68393a0` |
| Origin slug | `christian-velvet/tmp-0093db8b6deea44f` |
| Codebase | https://cursor.com/codebase/christian-velvet/tmp-0093db8b6deea44f |
| Clone | `https://origin.cursor.com/christian-velvet/tmp-0093db8b6deea44f.git` |
| v0.1.0 vendor | `origin-unreachable` |

HQ overlay: `CALENDAR.md` + `RHYTHM.md` + `LEDGER.md` + `HANDOFF-he.md` + `G003.md` + `G004.md` + `hq/FOLLOWER-GROWTH.md`. Do not move a booked board. Do not invent media. Schedule on instagram.com, not Meta Suite.

Source tree is not in this folder until `scripts/vendor-origin-packs.sh` succeeds.
Do not commit secrets. Do not invent prices. Do not send Instagram from this pack.

Embedded 30.8.2026 from Gemini share: `CALENDAR.md`.  
Perplexity: start with native IG schedule; paid tools only if lead opens.  
31.8.2026: floor-event → content table (ChatGPT Plus).
