...
Windows commissioning is performed through `scripts/bootstrap-media-host-windows.ps1`, which reuses the existing Windows Edge bootstrap and Remote Desktop Commander route. The Windows AMD host is CPU-backed for VoiceStudio because ROCm acceleration is Linux-only upstream.
