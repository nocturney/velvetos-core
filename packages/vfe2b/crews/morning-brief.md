...
4. Fill brief slots from verified sources only: run `python3 scripts/vfops_loop.py brief --write` (pulls vfgrowth / vfcopy / vfsku / vfcost / research). Also pipeline board, orchestration 06:15, calendar. Output three buckets in Hebrew: **היום** / **אדם** / **אחר כך**. Name **one** job the human can hand to `@vfe2b run` (Taskuary).
