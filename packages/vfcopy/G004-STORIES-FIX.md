# G004 · תיקון סטוריז — סיפור-מוצר

סטוריז `@velvets_cloud` · מחזיק טבעות בצורת קטלבל ורוד  
קול: **סיפור-מוצר** (`VOICE.md` כיתוב ב׳). לא תהליך-קצר. לא קופי דק. לא פתיחה במגבלה.

תלונת כריסטיאן 7.9 ~08:27: הסטוריז החיות נראו מתחת לרף סוכנות.  
Canva שנקרא היום (`DAHUaUo3bAk`): חידה + מגבלה על הפריים — קטלוג, לא סיפור.

מחיר: `X ₪` עד ראש צוות. **בלי ₪ בפריים ובכיתוב.**  
CTA: וואטסאפ `050-2517000` · איסוף שדרות. לא «שלחו DM».

`#vfigos` לסקירה. שיבוץ ב-instagram.com. **Cursor לא מפרסם.**

## פלטת פריים (מבריף בעלים ב-`vfcanva/jobs/G004.md`)

נייבי `#0B1D36` · זהב `#C9A86C` · קרם על כהה.  
טקסט על הפריים קצר. כיתוב ארוך נשאר כאן, לא על התמונה.

## ארבעה פריימים — להדבקה (20:30 א׳–ה׳)

סיפור-מוצר מקוצר. הוק אופי → היכרות → למי → הזמנה.

```
ורוד על השידה — והטבעות כבר במקום 💍

הכירו את מחזיק הטבעות שלנו בצורת קטלבל ורוד. הודפס בתלת־ממד, עם מקום למעלה לתכשיטים לפני האימון.

מתאים לפינת האימון בבית, כמתנה, או לכל מי שנמשך לצבע ולצורה.

רוצים אחד משלכם או בצבעים בהתאמה?
וואטסאפ 050-2517000 📲
היילייטס · איסוף שדרות
```

### על הפריים (קצר, נייבי-זהב)

| # | על המסך | לא על המסך |
|---|---|---|
| 1 | ורוד על השידה | חידה / הוק מגבלה |
| 2 | הטבעות למעלה לפני האימון | מגבלה כהוק |
| 3 | מתנה · פינת אימון · אופי | מק״ט / חומר / «קיים במלאי» |
| 4 | 050-2517000 · איסוף שדרות | ₪ / «שלחו DM» / «בלי משלוח» |

«לא משקולת» מותר **בגוף** אם צריך הבהרה — לא הוק, לא שורה 1.

## סדר מדיה (גלם מתיבת Grok — לא בגיט)

1. `kettlebells-pink-batch-2026-09-06-01`…`03` — אופי, בלי ספוילר ללקוח
2. `…-04` / `…-05` — עוד זווית
3. `kettlebells-pink-tops-print-end-2026-09-06` — תקריב: מקום לטבעת
4. CTA נייבי-זהב — וואטסאפ + איסוף

בלי סטיל מאותו יום = **לא משבצים**.

## מסירת Canva / vfcovers — בלי פיקסלים מומצאים

עיצובים קיימים (MCP `edit_url` בלבד — לא ממציאים קישור):

| פריים | id | עריכה |
|---|---|---|
| סטוריז 1–3 | `DAHUaUo3bAk` | https://www.canva.com/d/q03ula-BfH7vDn6 |
| סטורי 4 CTA | `DAHUacDGv9U` | https://www.canva.com/d/njntcYD-kdHr4VR |
| סט חלופי 4 עמודים | `DAHUacAsoNU` | https://www.canva.com/d/Kt13n_xcEcOBK6i |

**סטודיו עכשיו**

1. Canva MCP: `start-editing-transaction` → החלפת הטקסט בטבלת «על הפריים» → `commit`.
2. `export-design` type `png` · 1080×1920 · `export_quality=pro`.
3. אם Canva `needsAuth`: `python3 packages/vfcanva/studio/render.py --format ig_story` עם ההוק למעלה — או `vfcovers` compose אם יש גלם מקומי.
4. בלי `edit_url` / בלי PNG מורכב = השורה נשארת **חסום עריכה**. לא JPEG גולמי מהתיבה.

Cloud HQ לא כותב PNG לגיט. נתיב ייצוא קודם (6.9, פג): `/opt/cursor/artifacts/g004/`. לייצוא חוזר — MCP או File → Download בעורך.

קרוסלה פיד (אם עדיין המועמד ה׳ 12:00): נשארת `DAHUaelaug0` + כיתוב `G004.md` סיפור-מוצר. הסטוריז חייבות את התיקון הזה קודם.

## בדיקה לפני `#vfigos`

- [ ] מצב אחד: סיפור-מוצר (לא תהליך-קצר)
- [ ] ההוק חם — לא כותרת מדף, לא מגבלה
- [ ] אין ₪ / אין «שלחו DM» / אין «מוכנים» / אין «בלי משלוח» בשורה 1
- [ ] וואטסאפ `050-2517000` + איסוף שדרות בפריים 4
- [ ] עבר שער עריכה (Canva או vfcovers/vfcanva)
- [ ] pass: `hq/ai-tells-he.md`
