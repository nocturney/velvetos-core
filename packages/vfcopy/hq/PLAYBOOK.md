# לינט טיוטה

לפני טיוטה: `hq/reader-first-he.md` (שתי שאלות + קליטת למי/מדף/סיפור — מקור: mikiarlo3/ai-copywriter, מוטמע בפק, לא plugin).  
Pass אחרון: `hq/ai-tells-he.md` (סימני AI בעברית — מקור: plannotator/write-better + אותם דפוסי Humanizer, מוטמע בפק, לא plugin).

## אנטומיית פרומפט (מ־prompts.chat פרק 02)

לפני טיוטה — בנה/י מהרכיבים, לא «act as X» גנéric:

| רכיב | במשרד |
|---|---|
| **תפקיד** | סטודיו הדפסה · שדרות · איסוף בלבד |
| **הקשר** | שדות מפנייה: חומר, כמות, גימור, מתי — «חסר» אם אין מקור |
| **קורא** | תחושה ברגע + למי/מדף/סיפור מ־`reader-first-he.md` — גנרי מדי = שאל לפני טיוטה |
| **משימה** | פעולה אחת: תשובה / מעקב / כיתוב IG |
| **אילוצים** | CTA אחד · בלי ₪ · בלי DM · בלי דדליין לפני `#vfprod` |
| **פורmat** | טקסט גולמי / מבנה ריל — ראה `hq/templates/` |
| **דוגמה** | one-shot בתוך התבנית; לא מ־`prompts.csv` |

תבניות מוכנות: `hq/templates/README.md`. מקור מתודולוגיה: `packages/vfresearch/sources/2026-08-31-prompts-chat-embed.md`.

פרומפטי **תמונה/וידאו** חיצוניים (YouMind וכו׳): לא מחליפים את האנטומיה הזו — ראו `vfcovers/hq/PLAYBOOK.md` (Canva קודם; התאמת נושא לרצפה בלבד).

## בודקים

- עברית טבעית, בלי «נשמח לעמוד לשירותך»  
- יודעים מה הקורא מרגיש ברגע הזה (לא רק «מי הקהל»)  
- CTA אחד  
- וואטסאפ `050-2517000` ואיסוף שדרות כשזה מסר מכירה  
- אין ₪ אלא ממקור מאומת  
- אין הבטחת דדליין לפני `#vfprod`  
- אין קריאת «שלחו DM» שמעודדת דליפת פרופיל — מעדיפים וואטסאפ מפורש + היילייטס  
- כיתוב ריל: hook + משפט על הסרטון + כיתוב + CTA וואטסאפ/היילייטס; האשטאגים רק אם רלוונטיים, בלי רשימת ספאם  
- pass אחרון: `ai-tells-he.md` (בלי ultimate/game-changer/unlock בעברית או באנגלית)  

G005 חי (3.9). G003 נעול «מה יוצא מהמדפסת?» — משובץ 7.9 16:00. G004 = קטלבל-מחזיק (לא משקולת). לוח: `vfgrowth/CALENDAR.md`. המרת פרופיל: `vfgrowth/hq/FOLLOWER-GROWTH.md`. לא מיישמים על חשבון חי מכאן. `#vfigos` לסקירה; שיבוץ ב־instagram.com.

שפת לקוח משיחה: `@vfmakers brain capture` → `hq/customer-language.md`. `trust: unreviewed` עד שאדם מאשר.
