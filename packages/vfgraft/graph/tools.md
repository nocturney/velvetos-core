...
Gmail (`nocturney@gmail.com`) is read-only: search, get thread, labels. Calendar is `Asia/Jerusalem`; create only if asked. Drive is search-by-job — no dedicated studio folder. Canva is the Instagram visual tool. Superdesign for packet graphics. Treg after login; say the catalog price before `call`. Mobbin only when the MCP is actually on the agent.
