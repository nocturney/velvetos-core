...
רעיון / תמונה     vfresearch + vlicense + vfprod/3DAISTUDIO.md
  ...
  → באצ׳ / כרטיס   vfsku
  → טיוטת תוכן     vfgrowth + vfcopy + vfcovers
  → שיבוץ וחי      vfigos לסקירה · Grok שולח
```

מתחילים במה שכבר רץ היום (וואטסאפ → סלייס → סכום → Invoice4U → איסוף שדרות).  
חוליה חדשה רק אם ראש צוות פותח אותה בבריף `01`. לא בונים את כל השרשרת בבת אחת.
