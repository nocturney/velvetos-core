# מחקר מוזיקה לאינסטגרם — `@velvets_cloud`

מושב: **ייצור** (`@trend-researcher`) → מסירה ל־**צמיחה** (`@instagram-curator` / `@visual-storyteller`).  
לא פק חדש. HQ שולח דרך כלים (`constitution/SEND.md`). Treg לא בשימוש.

מטרה: להמליץ על **סאונד / מוזיקה פופולרית או מתאימה** לריל / סטורי / פוסט וידאו של Velvet Factory — מתוך מחקר טרנדים ושוק, לא מהמצאה.

## מי מטפל

| תפקיד | Mention | Pack | עושה | לא עושה |
|---|---|---|---|---|
| חוקר | `@trend-researcher` | `vfresearch` + `vfseason` | אוסף סיגנלים, כותב בריף מוזיקה | לא שולח, לא ממציא טרנד |
| סינתזה | `@research-synthesist` | `vfresearch` | מקורות + קישורי השראה שבועיים | לא ממציא גוף חסום |
| ריל | `@visual-storyteller` | `vfom` `vfcovers` | מתאים אנרגיה לקצב הקליפים מהרצפה | לא ממציא סצנת מיטה |
| לוח | `@instagram-curator` | `vfigos` | בודק התאמה לחבילה לפני שיבוץ | לא שולח / בוסט / DM |

מיומנות: `.cursor/skills/vf-ig-music/SKILL.md`  
מסגרת אודיו כללית (לא תחליף למחקר חי): `packages/vfmskill/vendor/social/references/short-form-video.md` → Audio Strategy.  
רישום מקורות: `SOURCES-MUSIC.json` + שורות ב־`LINKS.json`.

## Audio Gate — חלק מחבילת הווידאו, לא תוספת אופציונלית

כל Reel / Story וידאו חייב לקבל החלטת אודיו בזמן ה־Creative Manifest. `audioStrategy` אינו שדה תיעודי בלבד: לפני exact-final QA יש למדוד ולבדוק stream presence, loudness, clipping/noise, sync והתאמה לסיפור.

- audio stream חסר → `audio_repair`.
- stream קיים אבל near-silent → `audio_repair`.
- silence מותר רק כאשר הוא מכוון, מנומק ומאושר בקריאייטיב; שקט מקרי אינו PASS.
- source sound מועדף כשיש בו ערך ממשי של סטודיו/מכניקה.
- music/SFX צריכים לתמוך בהוק, proof, קצב ו־ending; לא רק “למלא שקט”.
- בחירת מוזיקה מתוך Instagram בזמן פרסום חייבת להיות מתועדת בבריף לפני השיבוץ ולאמת זמינות בחשבון בפועל.

## מתי

- **חובה** לפני `ready_for_publish` לכל ריל / סטורי וידאו שאין בו סאונד מקור שימושי וברמת loudness מתאימה.
- לפני חבילת תוכן עם ריל / סטורי וידאו / טיימלאפס.
- כשראש צוות שואל «איזו מוזיקה» / «סאונד טרנדי» / «מה רץ באינסטגרם».
- לא בכל בריף בוקר — רק כשיש וידאו בלוח או בקשה מפורשת.

## מקורות (בלי Treg)

**עזבו את Treg למשימת מוזיקה.** אין `treg login` / `call` לכאן.

סדר עדיפות:

1. **HeyOrca — מדריך שבועי (חינם, URL פתוח)**  
   https://www.heyorca.com/blog/trending-audio-for-reels-tiktok  
   מתעדכן בימי שישי. לקרוא את בלוק **Trending Instagram sounds this week** קודם. בלוק TikTok = סיגנל מוקדם בלבד (לא אסטרטגיית TikTok בלי ראש צוות). לרשום שם סאונד + התאמה לסטודיו + URL המאמר.

2. **אינסטגרם עצמה (העתקה מהבעלים / Grok)**  
   - Reels → אייקון תווים → לשונית **Trending**  
   - Professional dashboard → Tips and resources → **Trending audio** (עד ~50)  
   - חשבון `@creators` — דוח טרנדים שבועי  
   רק שמות שנמסרו במפורש מהמסך. אין ניחוש.

3. **מקור כתוב נוסף עם URL** שנרשם ב־`SOURCES-MUSIC.json` / `LINKS.json` (למשל מדריך שיטה PostPlanify). חומה / מנוי → «אין גוף», לא ממציאים.

4. **אין מקור** → בריף עם «חסר מקור חי» + המלצות **סגנון אנרגיה** בלבד (לא שמות שירים מומצאים).

אסור: להמציא שם טרנד, מספר שימושים, Insights, או «השיר הכי חם השבוע» בלי מקור.  
אסור: Butter / כלי בתשלום כמקור חובה (אם נפתח בחינם — לרשום; אם חומה — לדלג).

## התאמה לסטודיו (Velvet Factory)

| סוג תוכן | אנרגיה מועדפת | הערות |
|---|---|---|
| טיימלאפס הדפסה | בינונית–גבוהה, קצב יציב | לחתוך על שכבות / שינויי מצלמה; בלי מילים שחוסמות את הכיתוב |
| תוצאה / הוק מוצר | קצר, hook בשניות 0–3 | סאונד שעולה מהר; לא בלדה ארוכה |
| מאחורי הקלעים רצפה | רגוע / industrial-light | לא להסיח מסצנת אמת מהדרייב |
| סטורי קצר | טרנד קצר אם עדיין עולה | אם יורד — לדלג; לא רודפים טרנד מת |
| קרוסלה סטילס | אופציונלי / בלי מוזיקה | מוזיקה לא חובה לסטילס |

חוקי מותג:

- CTA ציבורי לפי `constitution/PUBLIC_CTA.md`; **אין WhatsApp / 050-2517000 / wa.me בתוכן ציבורי**.
- לא «שלחו DM» — בקופי public משתמשים בניסוח העברי שאושר ב־`PUBLIC_CTA.md`, ורק כאשר CTA בכלל רלוונטי.
- תוכן showcase/editorial אינו חייב CTA מכירתי; אם יש CTA, הוא יכול להיות follow/comment/שאלה או Instagram message ניטרלי לפי ההקשר והחוקה.
- אין ₪ על הפריים / בכותרת השיר.
- בלי מוזיקה מוגנת זכויות מחוץ לספריית הפלטפורמה — אלא אם ראש צוות פתח רישיון. חשבון Business = ספרייה מסחרית מצומצמת — לאמת ב־IG לפני שיבוץ.
- עברית בכיתוב; שם סאונד יכול להישאר כמו ב־IG.

## פלט — בריף מוזיקה

ארטיפקט: `packages/vfresearch/sources/YYYY-MM-DD-ig-music.md`

```markdown
# בריף מוזיקה IG · YYYY-MM-DD

מושב: ייצור · @trend-researcher · Asia/Jerusalem
חשבון: @velvets_cloud
פורמט יעד: ריל / סטורי / טיימלאפס (סמן אחד)

## מקורות
- [ ] HeyOrca weekly (תאריך עדכון במאמר): …
- [ ] צילום מסך / רשימה מהבעלים (IG Trending / Professional dashboard): …
- [ ] URL כתוב נוסף: …
- Treg: **לא בשימוש** למשימה זו
- אין מקור חי: כן / לא

## המלצות (רק עם מקור)
| # | שם סאונד / שיר | למה עכשיו | אנרגיה | מתאים ל | סיכון |
|---|---|---|---|---|---|
| 1 | … | ממקור X / עולה | … | טיימלאפס / הוק | Business library / יורד |

## בלי שמות (אם אין מקור)
- קצב / תחושה: …
- source / music / SFX / שילוב: …
- ducking מתחת ל־VO/סאונד מקור: כן/לא
- חלופה: ספריית IG «search terms» להעביר ל־Grok: …

## Audio QA
- stream presence: PASS / FAIL
- loudness: PASS / FAIL · measurement: …
- clipping/noise: PASS / FAIL
- sync: PASS / FAIL
- story-fit: PASS / FAIL
- intentional silence: no / yes + reason

## מסירה
- → `vfom` / `vfgrowth`: שורה בכרטיס קליפ
- → `vfigos`: סקירה לפני שיבוץ
- שליחה: **לא מ־HQ**
```

## מסירה

1. `@trend-researcher` כותב את הארטיפקט אחרי קריאת HeyOrca (או מקור חי אחר מהרשימה).
2. `@visual-storyteller` (`vfom`) רושם את בחירת האודיו בכרטיס הריל.
3. `@instagram-curator` (`vfigos`) מאשר בסקירה — לא מזיז `#משובץ`, לא בוסט.
4. אודיו עובר exact-final Audio Gate; חסר/near-silent אינו `ready_for_publish`.
5. כשנדרש IG-native audio, זמינות הסאונד נבדקת בחשבון בזמן השיבוץ/פרסום ומאומתת לאחר הפרסום.

## נעילות

- אין שליחת Instagram / Gmail / WhatsApp מ־HQ
- אין פק חדש לרעיון «סוכן מוזיקה»
- אין Insights / מספרי שימושים מומצאים
- אין שמות שירים מומצאים כשאין מקור
- **אין Treg** למחקר מוזיקה (מקורות: HeyOrca + IG מהבעלים + URL רשום)
- אין אסטרטגיית TikTok בלי ראש צוות (סיגנל מוקדם מותר עם ציון מקור)
