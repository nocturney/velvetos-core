...
# First-ever creation only: bootstrap is explicit and refuses to run if the service already exists.
# Before creation it skips only issuer-service Troubleshooter probes because that resource is absent;
# all project/secret/service-account/replay denials still run. The first revision is deployed at 0% traffic;
# after exact IAM replacement the script runs the full effective-IAM proof including issuer invoke/setIamPolicy.
# Only a passing full proof promotes traffic to latest. Any bootstrap failure before promotion deletes the
# unpromoted service, so signing secrets are never intentionally exposed by a failed bootstrap.
VELVET_DELIVERY_APPROVAL_BOOTSTRAP=1 ./packages/vfigos/approval/issuer/deploy-issuer.sh
# Routine redeploy after the issuer exists: omit bootstrap. This requires the current
# exact owner-only service IAM policy before Cloud Build, then replaces and verifies it again.
