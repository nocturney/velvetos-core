# vfprod

Production pack — print-floor and job tracking.

| | |
|---|---|
| Agent | https://cursor.com/agents/bc-cd4a5cde |
| bcId | `bc-cd4a5cde` |
| Origin slug | `christian-velvet/tmp-c9ca74be9225ac7d` |
| Codebase | https://cursor.com/codebase/christian-velvet/tmp-c9ca74be9225ac7d |
| Clone | `https://origin.cursor.com/christian-velvet/tmp-c9ca74be9225ac7d.git` |
| v0.1.0 vendor | `origin-unreachable` |

HQ overlay: `CHECKLIST.md` — four ticks before print + queue flags. No camera daemon.

Source tree is not in this folder until `scripts/vendor-origin-packs.sh` succeeds.
Do not commit secrets. Do not invent prices. Do not send Instagram from this pack.

Embedded 30.8.2026 from Gemini share: `FLOOR.md`, `MATERIAL.md`.  
Perplexity: Maxel farm → human bed assign; no auto farm from HQ.
