...
7. קו B2B נעול. אם ראש צוות פתח — תבנית מקומית ב־`hq/B2B-QUOTE.md` (לוגו/QR/מפיות = דוגמאות).
...
המסמך מחייב תהליך פנימי. **אין** שליחת הודעות אוטומטיות / בוטים / התחייבות חדשה מול לקוח בלי אדם ב־`050-2517000`.
...
הסולם (`vfharness` `run_ladder`) מוריד לטיוטה חלקית עם `חסר:` ו־`X ₪`. אין מחיר מומצא.
