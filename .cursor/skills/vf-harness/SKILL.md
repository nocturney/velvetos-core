...
5. Long task: write `packages/vfharness/state/<task-id>.json` from the checkpoint schema before you close. For a משמרת: `planned_steps` before heavy work; `gate` when blocked on ₪ or human field. See `playbooks/oma-patterns.md`.
