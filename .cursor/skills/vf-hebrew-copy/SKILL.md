...
description: >-
  ...
  quotes/proposals, captions, Reel/Story/Carousel copy, cover/overlay/first-frame text, documents,
  UI microcopy, anti-AI Hebrew QA, needs_input, or when text sounds like ChatGPT Hebrew.
...
9. Run factual/constraint validation after style. Humanizer must not soften or alter a real blocker/number/status.
10. Return `visible_text_gate: PASS` only if the relevant stages actually ran; otherwise `UNPROVEN`/`FAIL`.
11. For visual microcopy: 3–5 candidates + `NO_TEXT`; `TEXT_WINS` only if copy adds a specific point/payoff and beats the clean visual.
12. Hand off to the relevant next surface. This skill does not publish/send, does not auto-DM, and does not invent ₪.
