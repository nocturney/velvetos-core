...
7. Invoke `.cursor/skills/vf-creative-director/SKILL.md` for concept, 3-5 first-frame candidates, format genome, exact shot plan, shot gaps, EDL, overlays and cover directions. Do not ask the owner to choose routine creative options.
8. Invoke `.cursor/skills/vf-media-librarian/SKILL.md` to search/classify canonical Media Vault assets and write concrete asset refs, rights notes and exact gaps into the Manifest. If a critical physical shot is missing, stop only that blocked branch as `waiting_for_media`.
...
11. Invoke `.cursor/skills/vf-brand-guardian/SKILL.md` for deterministic + Brand/Hook/composition/Reality/Originality + subject/reference + artifact checks, feed continuity and repair instructions. Require Visual OS brandScore >=80 + CONTENT-RUBRIC >=20/25 + Content Contract + policy + rights + written PREFLIGHT.
