...
Read `packages/vfom/VISUAL-OS.md`, `VISUAL-DNA.json`, `FOUNDRY.json`, `CREATIVE-MANIFEST.schema.json`, `CONTENT-CONTRACT.schema.json`, `MOTION-PRESETS.md`, `FORMAT-GENOMES.md`, `packages/vfgrowth/CONTENT-RUBRIC.md`, `packages/vfgrowth/PREFLIGHT.md` and the active content/feed history from existing office-learning/vfinsights sources.
...
6. Check originality/fatigue against existing history. Reuse mechanics, not another creator's identity. High similarity must trigger a mechanic change, cooldown or documented series override.
7. Check feed continuity using the latest available 6-9 posts/covers. Balance proof/process/product/human/opinion without forcing a rigid checkerboard aesthetic.
8. For every failure, write evidence + exactly one smallest repair action. Route ordinary quality problems back to the Creative Director/Edit Director and re-score after repair.

## Pass criteria

Respect thresholds in `FOUNDRY.json`. Routine content requires at minimum Visual OS `brandScore >=80`, `CONTENT-RUBRIC >=20/25`, Content Contract pass, rights pass, policy pass and written PREFLIGHT. Hero/experimental work may require higher thresholds.

## Required manifest contribution

Update `qa.brandScore`, `qa.hookScore`, `qa.realityScore`, `qa.originalityScore`, `qa.artifactScore`, `qa.deterministicChecks`, `qa.failures`, `qa.repairs`, `feed.continuityDecision`, `feed.reasoning` and `status`.
