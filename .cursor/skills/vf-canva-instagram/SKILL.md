...
> LEGACY / provenance only for VF publication; not a provider route: Live visual tool for the Instagram page. Pack: `packages/vfcanva/`.
...
> LEGACY / provenance only for VF publication; not a provider route: - Do not construct Canva URLs. Use the `edit_url` the MCP returns.
...
> LEGACY / provenance only for VF publication; not a provider route: - Superdesign is fallback only when Canva MCP is down. Prefer `packages/vfcanva/studio/render.py` for a real PNG this HQ can produce without OAuth.

## Step 1 — Ticket

> LEGACY / provenance only for VF publication; not a provider route: Fill `packages/vfcanva/jobs/TEMPLATE.md` in the reply (do not invent a job name). Need:

1. Job / SKU / print the user named
> LEGACY / provenance only for VF publication; not a provider route: 2. Format id from `packages/vfcanva/FORMATS.json`
...
> LEGACY / provenance only for VF publication; not a provider route: ## Step 2 — Canva MCP gate

> LEGACY / provenance only for VF publication; not a provider route: Inspect the `Canva` MCP namespace (tools + auth status).

> LEGACY / provenance only for VF publication; not a provider route: - **`needsAuth` or no tools:** do not fake a design. Point to `packages/vfcanva/CONNECT.md`. Usual causes: **`spawn git ENOENT`** (marketplace plugin — Uninstall it, keep project `url` MCP), connecting from the cloud VM, Canva **Free**, or a team admin who disabled third-party integrations. Fallback: `packages/vfcanva/OPEN.md`.
- **Tools present:** continue. Use exact tool names from the live schema (discover before each call).

> LEGACY / provenance only for VF publication; not a provider route: Official Canva skills (read the matching file before mutating a design):

> LEGACY / provenance only for VF publication; not a provider route: - Edit / create on an existing design → `canva-edit-design` (transaction → operate → commit only after approval)
> LEGACY / provenance only for VF publication; not a provider route: - Resize → `canva-resize-for-social-media` using the `design_type` objects in `FORMATS.json`
> LEGACY / provenance only for VF publication; not a provider route: - Bulk SKU cards → `canva-bulk-create` (Enterprise autofill)
> LEGACY / provenance only for VF publication; not a provider route: - On-brand? → `canva-brand-check` (never invent a kit)
> LEGACY / provenance only for VF publication; not a provider route: - Critique → `canva-design-feedback`

## Step 3 — Make or reuse

> LEGACY / provenance only for VF publication; not a provider route: 1. If the user gave a design id (`D…`) or `canva.com/design/…` / `canva.link/…`, resolve that id. Do not search.
...
> LEGACY / provenance only for VF publication; not a provider route: Canva: <edit_url>
